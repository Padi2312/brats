...
class UNet3D(Module):
    def __init__(
        ...
        self.contracting_layers.append(
            double_conv_3d(self.in_channels, self.n_features[0])
        )
        ...
        self.bottleneck_layer = double_conv_3d(
            self.n_features[-1], self.n_features[-1] * 2
        )

        # generate expanding layers
        for n in reversed(self.n_features):
            self.expanding_layers.append(
                nn.ConvTranspose3d(n * 2, n, kernel_size=2, stride=2)
            )
...
if __name__ == "__main__":
    ...
    print("output shape x = " + str(model(x).shape))
